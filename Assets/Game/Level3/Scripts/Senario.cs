...
internal class Senario : MonoBehaviour
{
    ...
    internal void startEndFloor(float p1, float p2, float pass, int lines, int coluns)
    {
        ...
        GL.Vertex3((p2 - 3 * pass), p1 + (pass * lines / 2) + pass + pass / 2, 0);
        //end
        GL.Vertex3(((p2 + pass * coluns) - 3 * pass), p1 + (pass * lines ), 0);
        GL.Vertex3((p2 + pass * coluns), p1 + (pass * lines), 0);
        GL.Vertex3((p2 + pass * coluns), p1 + (pass * lines) + 3 * pass  , 0);
        GL.Vertex3(((p2 + pass * coluns) - 3 * pass), p1 + (pass * lines) +  3 * pass, 0);
    ...
    internal void walls(float p1, float p2, float pass, int lines, int coluns)
    {
        ...
        GL.Vertex3(p2 + pass * coluns + size, p1 + (pass * lines )  + 3 * pass + size, 0);
        GL.Vertex3(p2 + pass * coluns, p1 + (pass * lines )  + 3 * pass + size, 0);

        //6
        GL.Vertex3(p2 + pass * coluns - 3 * pass, p1 + pass * lines + 3 * pass, 0);
        GL.Vertex3(p2 + pass * coluns - 3 * pass, p1 + pass * lines + 3 * pass  + size, 0);
        GL.Vertex3(p2 + pass * coluns, p1 + pass * lines + 3 * pass  + size, 0);
        GL.Vertex3(p2 + pass * coluns, p1 + pass * lines + 3 * pass , 0);

        //7
        GL.Vertex3(p2 + pass * coluns - 3 * pass, p1 + (pass * lines), 0);
        GL.Vertex3(p2 + pass * coluns - 3 * pass  - size, p1 + (pass * lines), 0);
        GL.Vertex3(p2 + pass * coluns - 3 * pass  - size, p1 + (pass * lines) + 3 * pass + size, 0);
        GL.Vertex3(p2 + pass * coluns - 3 * pass, p1 + (pass * lines) + 3 * pass + size, 0);

        //8
        GL.Vertex3(p2 - size, p1 + pass * lines, 0);
        GL.Vertex3(p2 - size, p1 + pass * lines + size, 0);
        GL.Vertex3(p2 + pass * coluns - 3 * pass, p1 + pass * lines + size, 0);
        GL.Vertex3(p2 + pass * coluns - 3 * pass, p1 + pass * lines, 0);
        //9
        GL.Vertex3(p2, p1 + (pass * lines / 2) + pass + pass / 2, 0);
        GL.Vertex3(p2 - size, p1 + (pass * lines / 2) + pass + pass / 2, 0);
